...
- hosts: all
  ...
  tasks:
    ...
    - name: Ensure mango-task-2 is present
      ansible.builtin.file:
        path: /opt/iris/mango-task-2
        state: directory

    - name: Template mango-task-2 config
      ansible.builtin.template:
        src: templates/mango-task-2.j2
        dest: /etc/iris/mango-task-2.conf
        mode: '0644'

    # Description:
    # Duis aute irure dolor in reprehenderit in voluptate velit esse cillum. Curabitur non nulla sit amet nisl tempus convallis quis ac lectus. Integer nec odio. Praesent libero. Sed cursus ante dapibus diam. 

    - name: Ensure papa-task-3 is present
      ansible.builtin.file:
        path: /opt/iris/papa-task-3
        state: directory

    - name: Template papa-task-3 config
      ansible.builtin.template:
        src: templates/papa-task-3.j2
        dest: /etc/iris/papa-task-3.conf
        mode: '0644'
